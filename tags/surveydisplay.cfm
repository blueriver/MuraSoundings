...
<cfelse>
	<cfset perpage = request.pApp.settings.perpage>
</cfif>
<!--- how many pages? --->
...
<cfif isDefined("form.goback")>
	...
	<cfif currentInfo.currentStep gte 2>
		...
		<cfloop index="step" from="#firstOnPage#" to="#lastOnPage#">
			...
			<!--- Get current step --->
			<cfset question = request.pApp.question.getQuestions(attributes.survey.id,step)>
		...
		<cflocation url="./?#cgi.query_string#" addToken="false">
	</cfif>
</cfif>
