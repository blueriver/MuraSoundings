...
<cfif not attributes.getother>
	...
	<!--- get my answer data --->
	<cfset answers = request.pApp.question.getAnswers(attributes.questionidfk)>
	
	<cfloop query="answers">

		<cfquery name="getcount" datasource="#request.pApp.settings.dsn#">
			<!---
			select	count(r.answeridfk) as total
			from	#request.pApp.settings.tableprefix#results r,
					#request.pApp.settings.tableprefix#survey_results sr
			...
			and r.owneridfk in (
				select ownerid 
				from #request.pApp.settings.tableprefix#survey_results sr 
				where 1=1
				<cfif structKeyExists(attributes, "earliestdate") and isDate(attributes.earliestdate)>
				and		sr.completed >= <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.earliestdate#">
				</cfif>
	...
	<cfif attributes.other>
		...
		<cfquery name="getother" datasource="#request.pApp.settings.dsn#">
...
<cfelse>
	...
	<cfquery name="data" datasource="#request.pApp.settings.dsn#">
		select	r.other
		from	#request.pApp.settings.tableprefix#results r,
					#request.pApp.settings.tableprefix#survey_results sr
		where	r.questionidfk = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" maxlength="35" value="#attributes.questionidfk#">
